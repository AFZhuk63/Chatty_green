...
class VideoFeedView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        return VideoPost.objects.filter(author__in=subscribed_authors).select_related("author").prefetch_related("tags").order_by('-publication_date')  # Жадная загрузка # Если у постов есть ManyToMany или обратные связи (например, comments) используем ускоренную загрузку,
